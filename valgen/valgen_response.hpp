...
/// Defines ValgenResponseStatus, so valgen and its clients see the same values.

#pragma once

namespace ramfuzz {

/// Defines status values for valgen responses.
enum ValgenResponseStatus {
...
inline uint8_t status(ValgenResponseStatus s) { return s; }
